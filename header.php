...
<header
	...
	<div class="border-b border-black/5 bg-brand-bg">
		<div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
			<div class="flex h-[68px] items-center gap-4 lg:h-20 lg:gap-8">
				...
				<!-- Mobile menu -->
				<button @click="mobile = !mobile" class="-ml-1 p-2 text-brand-title lg:hidden" aria-label="<?php esc_attr_e( 'Menu', 'isdb-custom' ); ?>">
				...
				<div class="isdb-search relative hidden flex-1 lg:block">
					<form role="search" method="get" action="<?php echo esc_url( home_url( '/' ) ); ?>" class="search-form-main" autocomplete="off">
						<label for="wmb-search" class="sr-only"><?php esc_html_e( 'Search products', 'isdb-custom' ); ?></label>
						<div class="flex overflow-hidden rounded-full border border-brand-line bg-white focus-within:border-brand-primary">
							<input id="wmb-search" type="search" name="s" value="<?php echo get_search_query(); ?>"
								placeholder="<?php echo esc_attr( __( 'Search in…', 'isdb-custom' ) ); ?>"
							...
							<button type="submit" class="flex flex-none items-center px-5 text-brand-title transition hover:text-brand-primary" aria-label="<?php esc_attr_e( 'Search', 'isdb-custom' ); ?>">
				...
				<div class="ml-auto flex flex-none items-center gap-1 sm:gap-2 lg:gap-5">
					...
					<button @click="search = !search" class="p-2 text-brand-title lg:hidden" aria-label="<?php esc_attr_e( 'Search', 'isdb-custom' ); ?>">
					...
					<a href="<?php echo esc_url( isdb_track_order_url() ); ?>" class="hidden items-center gap-2 text-brand-title transition hover:text-brand-primary lg:flex">
						...
						<span class="flex flex-col leading-tight">
							<span class="text-[11px] text-slate-500"><?php esc_html_e( 'Track', 'isdb-custom' ); ?></span>
							<span class="text-sm font-semibold"><?php esc_html_e( 'Order', 'isdb-custom' ); ?></span>
						</span>
					</a>
					...
					<a href="<?php echo esc_url( $account_url ); ?>" class="hidden flex-col items-center gap-0.5 text-brand-title transition hover:text-brand-primary sm:flex">
						...
						<span class="hidden text-[11px] font-medium leading-tight lg:block"><?php echo $is_logged ? esc_html__( 'Account', 'isdb-custom' ) : esc_html__( 'Sign In', 'isdb-custom' ); ?></span>
					</a>

					<!-- Wishlist -->
					<a href="<?php echo esc_url( add_query_arg( 'wishlist', '1', $account_url ) ); ?>" class="hidden flex-col items-center gap-0.5 text-brand-title transition hover:text-brand-primary sm:flex" aria-label="<?php esc_attr_e( 'Wishlist', 'isdb-custom' ); ?>">
						...
						<span class="hidden text-[11px] font-medium leading-tight lg:block"><?php esc_html_e( 'Wishlist', 'isdb-custom' ); ?></span>
					</a>

					<!-- Cart -->
					<button @click="cartOpen = true" class="cart-dropdown flex flex-col items-center gap-0.5 text-brand-title transition hover:text-brand-primary" aria-label="<?php esc_attr_e( 'Open cart', 'isdb-custom' ); ?>">
						<span class="relative">
						...
						<span class="hidden text-[11px] font-medium leading-tight lg:block"><?php esc_html_e( 'Cart', 'isdb-custom' ); ?></span>
					</button>
			...
			<div x-show="search" x-cloak class="isdb-search relative pb-3 lg:hidden">
				<form role="search" method="get" action="<?php echo esc_url( home_url( '/' ) ); ?>" autocomplete="off">
					<div class="flex overflow-hidden rounded-full border border-brand-line bg-white focus-within:border-brand-primary">
						<label for="wmb-search-m" class="sr-only"><?php esc_html_e( 'Search products', 'isdb-custom' ); ?></label>
						<input id="wmb-search-m" type="search" name="s" placeholder="<?php echo esc_attr( __( 'Search in…', 'isdb-custom' ) ); ?>" class="isdb-search-input w-full border-0 bg-transparent px-4 py-2.5 text-sm focus:outline-none focus:ring-0" />
						<input type="hidden" name="post_type" value="product" />
						<button type="submit" class="flex-none px-4 text-brand-title" aria-label="<?php esc_attr_e( 'Search', 'isdb-custom' ); ?>">
							<svg class="h-5 w-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-5.197-5.197m0 0A7.5 7.5 0 105.196 5.196a7.5 7.5 0 0010.607 10.607z"/></svg>
						</button>
					</div>
				</form>
	...
	<div class="header-bottom hidden bg-brand-dark lg:block">
		<div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
			<div class="flex h-12 items-center justify-between">
				...
				<a href="<?php echo esc_url( isdb_phone_tel() ); ?>" class="flex items-center gap-2 text-sm font-semibold text-white/90 transition hover:text-brand-primary">
	...
	<div x-show="mobile" x-cloak class="fixed inset-0 z-[80] lg:hidden" role="dialog" aria-modal="true" aria-label="<?php esc_attr_e( 'Menu', 'isdb-custom' ); ?>">
			...
			<div class="flex items-center gap-3 bg-brand-primary px-5 py-4 text-white">
				...
				<div class="min-w-0 flex-1">
					<?php if ( $is_logged ) : $current_user = wp_get_current_user(); ?>
						...
						<a href="<?php echo esc_url( wp_logout_url( home_url( '/' ) ) ); ?>" class="text-xs text-white/80 transition hover:text-white"><?php esc_html_e( 'Logout', 'isdb-custom' ); ?></a>
					<?php else : ?>
						<p class="text-sm font-bold leading-tight"><?php esc_html_e( 'Welcome', 'isdb-custom' ); ?></p>
						<a href="<?php echo esc_url( $account_url ); ?>" class="text-xs text-white/80 transition hover:text-white"><?php esc_html_e( 'Sign In / Register', 'isdb-custom' ); ?></a>
					<?php endif; ?>
				</div>
				<button @click="mobile=false" class="-mr-1 flex-none rounded-full p-1.5 text-white/80 transition hover:bg-white/15 hover:text-white" aria-label="<?php esc_attr_e( 'Close menu', 'isdb-custom' ); ?>">
			...
			<div class="flex-1 overflow-y-auto">
				...
				<div class="grid grid-cols-3 divide-x divide-slate-100 border-b border-slate-100 text-center">
					<a href="<?php echo esc_url( $account_url ); ?>" class="flex flex-col items-center gap-1 px-2 py-3.5 text-brand-title transition hover:bg-brand-soft">
						<svg class="h-6 w-6" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M15.75 6a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0zM4.501 20.118a7.5 7.5 0 0114.998 0A17.933 17.933 0 0112 21.75c-2.676 0-5.216-.584-7.499-1.632z"/></svg>
						<span class="text-[11px] font-semibold"><?php echo $is_logged ? esc_html__( 'Account', 'isdb-custom' ) : esc_html__( 'Sign In', 'isdb-custom' ); ?></span>
					</a>
					<a href="<?php echo esc_url( add_query_arg( 'wishlist', '1', $account_url ) ); ?>" class="flex flex-col items-center gap-1 px-2 py-3.5 text-brand-title transition hover:bg-brand-soft">
						<svg class="h-6 w-6" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12z"/></svg>
						<span class="text-[11px] font-semibold"><?php esc_html_e( 'Wishlist', 'isdb-custom' ); ?></span>
					</a>
					<button @click="mobile=false; cartOpen=true" class="flex flex-col items-center gap-1 px-2 py-3.5 text-brand-title transition hover:bg-brand-soft" aria-label="<?php esc_attr_e( 'Open cart', 'isdb-custom' ); ?>">
						<span class="relative">
							<svg class="h-6 w-6" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M15.75 10.5V6a3.75 3.75 0 10-7.5 0v4.5m11.356-1.993l1.263 12c.07.665-.45 1.243-1.119 1.243H4.25a1.125 1.125 0 01-1.12-1.243l1.264-12A1.125 1.125 0 015.513 7.5h12.974c.576 0 1.059.435 1.119 1.007z"/></svg>
						...
						<span class="text-[11px] font-semibold"><?php esc_html_e( 'Cart', 'isdb-custom' ); ?></span>
					</button>
				</div>
				...
				<!-- Categories -->
				<p class="px-5 pb-1 pt-4 text-[11px] font-bold uppercase tracking-wider text-slate-400"><?php esc_html_e( 'Categories', 'isdb-custom' ); ?></p>
				<?php isdb_primary_nav( 'mobile' ); ?>

				<!-- Quick links -->
				<p class="px-5 pb-1 pt-4 text-[11px] font-bold uppercase tracking-wider text-slate-400"><?php esc_html_e( 'Quick Links', 'isdb-custom' ); ?></p>
				<ul class="flex flex-col gap-0.5 p-2 pb-6 [&_a]:flex [&_a]:items-center [&_a]:gap-2.5 [&_a]:rounded-lg [&_a]:px-4 [&_a]:py-2.5 [&_a]:text-[14px] [&_a]:font-medium [&_a]:text-brand-body [&_a:hover]:bg-brand-soft [&_a:hover]:text-brand-primary">
					<li><a href="<?php echo esc_url( home_url( '/about-us/' ) ); ?>"><?php esc_html_e( 'About Us', 'isdb-custom' ); ?></a></li>
					<li><a href="<?php echo esc_url( add_query_arg( 'wishlist', '1', $account_url ) ); ?>"><?php esc_html_e( 'Wishlist', 'isdb-custom' ); ?></a></li>
					<li><a href="<?php echo esc_url( home_url( '/faqs/' ) ); ?>"><?php esc_html_e( 'FAQs', 'isdb-custom' ); ?></a></li>
				</ul>
			</div>
		</div>
	</div>
	...
	<!-- ============================ MINI-CART DRAWER ============================ -->
	<div x-show="cartOpen" x-cloak class="fixed inset-0 z-[60]" role="dialog" aria-modal="true" aria-label="<?php esc_attr_e( 'Shopping cart', 'isdb-custom' ); ?>">
		...
		<div x-show="cartOpen"
			...
			<div class="flex items-center justify-between border-b border-slate-100 px-5 py-4">
				<h2 class="flex items-center gap-2 text-base font-bold text-brand-title">
					...
					<?php esc_html_e( 'Shopping Cart', 'isdb-custom' ); ?>
				</h2>
				<button @click="cartOpen=false" class="rounded p-1.5 text-slate-400 transition hover:bg-stone-100 hover:text-brand-title" aria-label="<?php esc_attr_e( 'Close cart', 'isdb-custom' ); ?>">
					<svg class="h-6 w-6" fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/></svg>
				</button>
			</div>
