...
<main id="site-main" class="bg-stone-50 text-slate-900 antialiased">
	<div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8 lg:py-12">
		...
		<nav class="mb-6 text-sm text-slate-500" aria-label="<?php esc_attr_e( 'Breadcrumb', 'isdb-custom' ); ?>">
				...
				<div class="relative overflow-hidden rounded-3xl bg-white ring-1 ring-slate-100 shadow-sm">
					...
					<?php if ( $discount_pct > 0 ) : ?>
						<span class="absolute left-4 top-4 z-10 rounded-full bg-rose-600 px-3 py-1 text-xs font-bold uppercase tracking-wide text-white shadow-lg">
							<?php printf( esc_html__( 'Save %s%%', 'isdb-custom' ), esc_html( $discount_pct ) ); ?>
						</span>
					<?php endif; ?>

					<?php
					// Product image, with WooCommerce placeholder fallback when none is set.
					if ( $main_id ) {
						...
						echo wp_kses_post( wc_placeholder_img( 'large', array(
							'id'    => 'wmb-main-image',
			...
			<div class="flex flex-col">
				...
				<?php if ( $high_demand ) : ?>
					<div class="mb-3 inline-flex w-fit items-center gap-2 rounded-full bg-rose-50 px-3 py-1 text-xs font-semibold text-rose-700">
						...
						<?php esc_html_e( 'High demand — selling fast this week', 'isdb-custom' ); ?>
					</div>
				<?php endif; ?>
				...
				<?php if ( $review_count > 0 ) : ?>
					<div class="mt-3 flex items-center gap-3">
						<div class="flex items-center" aria-label="<?php echo esc_attr( sprintf( __( 'Rated %s out of 5', 'isdb-custom' ), $avg_rating ) ); ?>">
							<?php for ( $s = 1; $s <= 5; $s++ ) :
				...
				<div class="mt-5 flex items-end gap-3">
					...
					<?php if ( $discount_pct > 0 ) : ?>
						<span class="mb-1 rounded-md bg-emerald-50 px-2 py-0.5 text-sm font-semibold text-emerald-700">
							<?php printf( esc_html__( 'You save %s%%', 'isdb-custom' ), esc_html( $discount_pct ) ); ?>
						</span>
					<?php endif; ?>
				</div>
				...
				<?php if ( $low_stock ) :
					...
					<div class="mt-6 rounded-2xl border border-rose-100 bg-rose-50/60 p-4">
						<div class="flex items-center justify-between text-sm font-semibold text-rose-700">
							<span>🔥 <?php printf( esc_html__( 'Only %s left in stock', 'isdb-custom' ), esc_html( $stock_qty ) ); ?></span>
							<span class="text-rose-500"><?php esc_html_e( 'Order soon', 'isdb-custom' ); ?></span>
						</div>
				...
				<?php elseif ( $in_stock ) : ?>
					<p class="mt-6 inline-flex w-fit items-center gap-2 text-sm font-semibold text-emerald-700">
						...
						<?php esc_html_e( 'In stock & ready to ship', 'isdb-custom' ); ?>
					</p>
				<?php endif; ?>
				...
				<div class="mt-7">
					<?php if ( $in_stock && $product->is_type( 'simple' ) ) : ?>
						<form class="wmb-cart flex flex-col sm:flex-row gap-3" action="<?php echo esc_url( wc_get_cart_url() ); ?>" method="post">
							<div class="flex items-center rounded-xl ring-1 ring-slate-200 bg-white">
								...
								<button type="button" class="wmb-qplus px-4 py-3 text-lg text-slate-500 hover:text-slate-900" aria-label="<?php esc_attr_e( 'Increase quantity', 'isdb-custom' ); ?>">+</button>
							</div>
					...
					<?php else : ?>
						<button type="button" disabled class="w-full cursor-not-allowed rounded-xl bg-slate-200 px-8 py-4 font-bold text-slate-500">
							<?php esc_html_e( 'Currently Unavailable', 'isdb-custom' ); ?>
						</button>
					<?php endif; ?>
				</div>
				...
				<div class="mt-8 grid grid-cols-2 sm:grid-cols-4 gap-4 border-t border-slate-100 pt-8">
					...
					$trust = array(
						array( 'M9 12.75L11.25 15 15 9.75m-3-7.036A11.959 11.959 0 013.598 6 11.99 11.99 0 003 9.749c0 5.592 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.285z', __( 'Secure Checkout', 'isdb-custom' ), __( 'SSL encrypted', 'isdb-custom' ) ),
						array( 'M2.25 18.75a60.07 60.07 0 0115.797 2.101c.727.198 1.453-.342 1.453-1.096V18.75M3.75 4.5v.75A.75.75 0 013 6h-.75m0 0v-.375c0-.621.504-1.125 1.125-1.125H20.25M2.25 6v9m18-10.5v.75c0 .414.336.75.75.75h.75m-1.5-1.5h.375c.621 0 1.125.504 1.125 1.125v9.75c0 .621-.504 1.125-1.125 1.125h-.375m1.5-1.5H21a.75.75 0 00-.75.75v.75m0 0H3.75m0 0h-.375a1.125 1.125 0 01-1.125-1.125V15m1.5 1.5v-.75A.75.75 0 003 15h-.75M15 10.5a3 3 0 11-6 0 3 3 0 016 0z', __( 'Cash on Delivery', 'isdb-custom' ), __( 'Pay when you get it', 'isdb-custom' ) ),
						array( 'M9 15L3 9m0 0l6-6M3 9h12a6 6 0 010 12h-3', __( 'Easy Returns', 'isdb-custom' ), __( '7-day replacement', 'isdb-custom' ) ),
				...
				<div class="mt-6 flex items-start gap-4 rounded-2xl bg-slate-900 p-5 text-white">
					...
					<div>
						<p class="font-bold"><?php esc_html_e( 'Way More BD — Customer Support Promise', 'isdb-custom' ); ?></p>
						<p class="mt-1 text-sm text-slate-300"><?php esc_html_e( 'Your trusted kitchen companion. Authentic products, honest pricing, and a team that answers. Not satisfied? We make it right.', 'isdb-custom' ); ?></p>
					</div>
				</div>
			</div>
		</div>
		...
		<div class="mt-16" x-data="{ tab: 'desc' }">
			...
			<div class="mb-4 flex flex-wrap gap-2">
				<button @click="tab='desc'"
					...
					class="rounded-lg px-5 py-2.5 text-sm font-semibold transition"><?php esc_html_e( 'Description', 'isdb-custom' ); ?></button>
				<button @click="tab='details'"
					...
					class="rounded-lg px-5 py-2.5 text-sm font-semibold transition"><?php esc_html_e( 'Specifications', 'isdb-custom' ); ?></button>
				<button @click="tab='reviews'"
					...
					class="rounded-lg px-5 py-2.5 text-sm font-semibold transition"><?php printf( esc_html__( 'Customer Reviews (%s)', 'isdb-custom' ), esc_html( $review_count ) ); ?></button>
			</div>
			...
			<div class="rounded-2xl bg-white p-6 ring-1 ring-slate-100 sm:p-8">
				...
				<div x-show="tab==='desc'">
					<h3 class="mb-5 inline-block border-b-2 border-brand-primary pb-1.5 text-base font-bold text-slate-900"><?php esc_html_e( 'Product Details', 'isdb-custom' ); ?></h3>
				...
				<div x-show="tab==='details'" x-cloak>
					<h3 class="mb-5 inline-block border-b-2 border-brand-primary pb-1.5 text-base font-bold text-slate-900"><?php esc_html_e( 'Specifications', 'isdb-custom' ); ?></h3>
					<?php
					...
					<?php else : ?>
						<p class="text-slate-500"><?php esc_html_e( 'No additional specifications listed for this product.', 'isdb-custom' ); ?></p>
					<?php endif; ?>
				</div>

				<div id="reviews" x-show="tab==='reviews'" x-cloak>
					<h3 class="mb-5 inline-block border-b-2 border-brand-primary pb-1.5 text-base font-bold text-slate-900"><?php esc_html_e( 'Customer Reviews', 'isdb-custom' ); ?></h3>
		...
		if ( $related ) : ?>
			<section class="mt-16">
				<?php isdb_section_head( __( 'Related Products', 'isdb-custom' ), get_permalink( wc_get_page_id( 'shop' ) ) ?: home_url( '/shop/' ), __( 'More Products', 'isdb-custom' ) ); ?>
