...
<h2 class="!mb-1 text-lg font-bold text-brand-title"><?php esc_html_e( 'Dashboard', 'isdb-custom' ); ?></h2>
<p class="m-0 text-[13px] text-slate-500"><?php esc_html_e( 'Here\'s what\'s happening with your account.', 'isdb-custom' ); ?></p>
...
<div class="mt-5 grid grid-cols-2 gap-3 lg:grid-cols-3">
	...
	$tiles = array(
		array(
			'label' => __( 'Total Orders', 'isdb-custom' ),
		...
		array(
			'label' => __( 'In Progress', 'isdb-custom' ),
			'value' => number_format_i18n( $processing_count ),
			'icon'  => 'M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z',
			'tone'  => 'text-amber-700 bg-amber-50',
		),
		array(
			'label' => __( 'Total Spent', 'isdb-custom' ),
...
<?php if ( $last ) :
	...
	<div class="mt-4 flex flex-col items-start justify-between gap-3 rounded-card p-4 sm:flex-row sm:items-center"
		...
		<div>
			<p class="m-0 text-sm font-bold text-brand-title"><?php esc_html_e( 'Order again in one click', 'isdb-custom' ); ?></p>
			<p class="m-0 text-[12px] text-slate-500">
				<?php esc_html_e( 'Last order', 'isdb-custom' ); ?> <span class="font-semibold text-brand-title">#<?php echo esc_html( $last->get_order_number() ); ?></span>
		...
		<a href="<?php echo esc_url( $reorder_url ); ?>"
			...
			<?php esc_html_e( 'Reorder', 'isdb-custom' ); ?>
...
<div class="mt-6">
	<div class="mb-3 flex items-center justify-between border-b border-[#ddd] pb-2.5">
		<h3 class="m-0 text-base font-bold text-brand-title"><?php esc_html_e( 'Recent Orders', 'isdb-custom' ); ?></h3>
		<a href="<?php echo esc_url( wc_get_account_endpoint_url( 'orders' ) ); ?>" class="text-[12px] font-semibold uppercase tracking-wide text-brand-primary transition hover:underline"><?php esc_html_e( 'View all', 'isdb-custom' ); ?></a>
	</div>
			...
			<?php foreach ( $recent as $o ) :
				...
				<a href="<?php echo esc_url( $o->get_view_order_url() ); ?>"
					...
					<span class="min-w-0 flex-1">
						...
						<span class="block text-[12px] text-slate-500"><?php echo esc_html( wc_format_datetime( $o->get_date_created() ) ); ?> &middot; <?php printf( esc_html( _n( '%d item', '%d items', $o->get_item_count(), 'isdb-custom' ) ), $o->get_item_count() ); ?></span>
					</span>
	...
	<?php else : ?>
		<div class="rounded-card border border-dashed border-[#ddd] bg-brand-bg py-10 text-center">
			<p class="m-0 text-sm font-semibold text-brand-title"><?php esc_html_e( 'No orders yet', 'isdb-custom' ); ?></p>
			<p class="m-0 mt-1 text-[12px] text-slate-500"><?php esc_html_e( 'Your orders will appear here once you place one.', 'isdb-custom' ); ?></p>
			<a href="<?php echo esc_url( wc_get_page_permalink( 'shop' ) ); ?>" class="mt-4 inline-block rounded-card bg-brand-primary px-5 py-2.5 text-[13px] font-bold text-white transition hover:bg-brand-hover"><?php esc_html_e( 'Start Shopping', 'isdb-custom' ); ?></a>
		</div>
	<?php endif; ?>
</div>
...
<!-- Quick actions -->
<div class="mt-6">
	<h3 class="mb-3 border-b border-[#ddd] pb-2.5 text-base font-bold text-brand-title"><?php esc_html_e( 'Quick Actions', 'isdb-custom' ); ?></h3>
	<div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
		<?php
		$actions = array(
			array( 'orders', __( 'My Orders', 'isdb-custom' ), __( 'Track & review your order history', 'isdb-custom' ), 'M8.25 6.75h12M8.25 12h12m-12 5.25h12M3.75 6.75h.007v.008H3.75V6.75zM3.75 12h.007v.008H3.75V12zm0 5.25h.007v.008H3.75v-.008z' ),
			array( 'edit-address', __( 'Addresses', 'isdb-custom' ), __( 'Manage billing & delivery details', 'isdb-custom' ), 'M15 10.5a3 3 0 11-6 0 3 3 0 016 0z M19.5 10.5c0 7.142-7.5 11.25-7.5 11.25S4.5 17.642 4.5 10.5a7.5 7.5 0 1115 0z' ),
			array( 'edit-account', __( 'Account Details', 'isdb-custom' ), __( 'Name, email & password', 'isdb-custom' ), 'M15.75 6a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0zM4.501 20.118a7.5 7.5 0 0114.998 0A17.933 17.933 0 0112 21.75c-2.676 0-5.216-.584-7.499-1.632z' ),
			array( 'customer-logout', __( 'Log out', 'isdb-custom' ), __( 'Sign out of your account securely', 'isdb-custom' ), 'M15.75 9V5.25A2.25 2.25 0 0013.5 3h-6a2.25 2.25 0 00-2.25 2.25v13.5A2.25 2.25 0 007.5 21h6a2.25 2.25 0 002.25-2.25V15M12 9l-3 3m0 0l3 3m-3-3h12.75' ),
		);
